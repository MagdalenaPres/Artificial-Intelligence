...
public class Game
{
    ...
    int visitedNodesForWhitePlayer = 0;
    ...
    public void run(Player whitePlayer, Player blackPlayer, Board board, int scoreCountHeuristic)
    {
        ...
        else
        {
            ...
            if(((BotPlayer) currentPlayer).isWithMinMax())
            {
                botActions.minMax(currentPlayer, oppositePlayer, searchDepth, scoreCountHeuristic, searchDepth);

                if(currentPlayer.isWhite()) visitedNodesForWhitePlayer = botActions.getVisitedNodesMinMax();
                else visitedNodesForBlackPlayer = botActions.getVisitedNodesMinMax();
            }
            else
            {
                botActions.alphaBetaPruning(currentPlayer, oppositePlayer, searchDepth, scoreCountHeuristic,
                        searchDepth, Integer.MIN_VALUE, Integer.MAX_VALUE);

                if(currentPlayer.isWhite()) visitedNodesForWhitePlayer = botActions.getVisitedNodesAlphaBetaPruning();
                else visitedNodesForBlackPlayer = botActions.getVisitedNodesAlphaBetaPruning();
            }
    ...
    {
        if(roundPlayedByWhiteBot > 0)
            return visitedNodesForWhitePlayer/roundPlayedByWhiteBot;
        else
            return 0;
    }
